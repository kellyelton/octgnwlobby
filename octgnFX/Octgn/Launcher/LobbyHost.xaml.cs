...
namespace Octgn.Launcher
{
    ...
    private bool isLobbyGame = false;
    public LobbyHost(bool isLobbyGame)
		{
			InitializeComponent();
            this.isLobbyGame = isLobbyGame;
			portBox.Text = Settings.Default.ServePort;
            nickBox.Text = Settings.Default.NickName;
            if (isLobbyGame)
            {
                nickBox.Text = Program.LClient.strUserName;
            ...
            if (Program.LClient != null)
            {
                if (Program.LClient.Connected && isLobbyGame)
                {
                    ...
                    if (isIPv6)
                    {
                        ...
                        {
                            str += ipList.Items[i].ToString();
                            if (i != ipList.Items.Count - 1)
                                str += '?';
                        }
                    }
                    else
                    {
                        for (int i = 0; i < ipv4List.Items.Count; i++)
                        {
                            str += ipv4List.Items[i].ToString();
                            if (i != ipv4List.Items.Count - 1)
                                str += '?';
                        }
                        str += '?' + externalIPCache;
                    }
                    //MessageBox.Show(str);
                    Program.LClient.Host_Game(str, port, Program.Game.Definition);
