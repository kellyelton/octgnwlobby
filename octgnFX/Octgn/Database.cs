...
namespace Octgn
{
    ...
    { get { return openedGame; } }
		...
		public static IEnumerable<CardModel> GetCards(Script.PropertyFilter filter)
		{
			string sql = string.Format("Card.[{0}] {1} '{2}'", filter.Name, filter.Negate ? "<>" : "=", filter.Value);
			return openedGame.SelectCardModels(new string[] { sql });
		}
